...
for file in $FILES
do
        if ! awk 'NR == 1 {print FILENAME}' $file | grep ".xyz" > /dev/null #check if it is .xyz file
        then
                continue
        fi
        ...
        awk '(match(FILENAME,".xyz")) && NR > '$lastcoord' {print($1, " ", $2, " ", $3, " ", $4)}' $file >> $filename.com #pasting coords without other junk
